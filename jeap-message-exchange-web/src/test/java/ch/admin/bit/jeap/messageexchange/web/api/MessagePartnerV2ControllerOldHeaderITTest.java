...
class MessagePartnerV2ControllerOldHeaderITTest {
    ...
    void getMessage_withoutAuthentication_thenReturnsUnauthorized() throws Exception {
        ...
        mockMvc.perform(
                        get("/api/partner/v2/messages/{messageId}", messageId)
                                .header("bpId", UUID.randomUUID().toString())
                                .contentType(MediaType.APPLICATION_XML_VALUE))
                .andExpect(status().isUnauthorized());
    ...
    void getMessage_whenAuthorizedForAllBpIds_thenAccessGranted() throws Exception {
        UUID messageId = UUID.randomUUID();
        String requerstBpId = UUID.randomUUID().toString(); // any business partner id
        MessageContent messageContent = new MessageContent(new ByteArrayInputStream("123".getBytes(UTF_8)), 3);
        when(messageExchangeService.getMessageContentFromInternalApplication(requerstBpId, messageId))
                .thenReturn(Optional.of(messageContent));

        mockMvc.perform(
                        get("/api/partner/v2/messages/{messageId}", messageId)
                                .header("bpId", requerstBpId)
                                .contentType(MediaType.APPLICATION_XML_VALUE)
    ...
    @Test
    void getMessages_whenRequestOk_thenReturnsResults() throws Exception {
        String bpId = UUID.randomUUID().toString();
        ...
        when(messageExchangeService.getMessages(bpId, null, null, null, null, null, 1000))
                .thenReturn(List.of(new MessageSearchResultDto(message1Id, "type1", MediaType.APPLICATION_XML_VALUE, "groupId1", "partnerTopic1", null, null),
    ...
    void getMessages_whenRequestWithAllParamsOk_thenReturnsResults() throws Exception {
        ...
        when(messageExchangeService.getMessages(bpId, topicName, groupId, lastMessageId, partnerTopic, null, size))
                .thenReturn(List.of(new MessageSearchResultDto(messageId, "type", MediaType.APPLICATION_XML_VALUE, "groupId", "partnerTopic", null, null)));
    ...
    void getNextMessage_whenRequestOk_thenReturnsResult() throws Exception {
        ...
        mockMvc.perform(
                ...
                .andExpect(header().string("message-id", messageIdFromDatabase.toString()))
                .andExpect(header().string("messageId", messageIdFromDatabase.toString()));
