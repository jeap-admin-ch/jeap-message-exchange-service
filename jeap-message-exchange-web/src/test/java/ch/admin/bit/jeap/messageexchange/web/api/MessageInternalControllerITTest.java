...
class MessageInternalControllerITTest {
    ...
    void getMessageFromPartner_messageScanFailed_thenReturnsForbidden() throws Exception {
        UUID messageId = UUID.randomUUID();
        doThrow(MalwareScanFailedOrBlockedException.malwareScanFailedOrBlockedException(messageId, ScanStatus.SCAN_FAILED)).when(messageExchangeService).getMessageFromPartner(messageId);
        mockMvc.perform(
                        get("/api/internal/v2/messages/{messageId}", messageId)
                                .contentType(MediaType.APPLICATION_XML_VALUE)
                                .with(authentication(createAuthenticationForUserRoles(B2B_MESSAGE_IN_READ))))
