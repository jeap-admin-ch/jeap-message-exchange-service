...
@WebMvcTest(controllers = {MessagePartnerV3Controller.class})
...
class MessagePartnerV3ControllerITTest {
    ...
    private ResultActions performPutMessage(String messageId, String bpId, String messageType, String xmlContent, Authentication authentication) {
        return mockMvc.perform(
                put("/api/partner/v3/messages/{messageId}", messageId)
    ...
    void getMessage_bpIdMissing_thenReturnsBadRequest() throws Exception {
        ...
        mockMvc.perform(
                        get("/api/partner/v3/messages/{messageId}", messageId)
                                .contentType(MediaType.APPLICATION_XML_VALUE)
    ...
    void getMessages_whenRequestOkWithTrailingSlash_thenReturnsResults() throws Exception {
        ...
        mockMvc.perform(
                        get("/api/partner/v3/messages/")
                                ...
                                .with(authentication(createAuthenticationForBpRoles(bpId, B2B_MESSAGE_OUT_READ))))
                .andExpect(status().isOk())
                .andExpect(content().string("<messages><message><messageId>" + message1Id +
                        "</messageId><messageType>type1</messageType></message><message><messageId>" + message2Id +
                        "</messageId><messageType>type2</messageType><groupId>groupdId2</groupId><partnerTopic>partnerTopic2</partnerTopic></message></messages>"));
    ...
    void getNextMessage_bpIdMissing_thenReturnsBadRequest() throws Exception {
        ...
        mockMvc.perform(
                        get("/api/partner/v3/messages/{messageId}/next", messageId)
                                .contentType(MediaType.APPLICATION_XML_VALUE)
                                .with(authentication(createAuthenticationForBpRoles(UUID.randomUUID().toString(), B2B_MESSAGE_OUT_READ))))
                .andExpect(status().isBadRequest());
