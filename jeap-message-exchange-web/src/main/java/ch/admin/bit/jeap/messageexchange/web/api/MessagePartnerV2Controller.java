...
/**
 * @deprecated Replaced with {@link MessagePartnerV3Controller}
 */
@RestController
...
@Deprecated // replaced by MessagePartnerV3Controller
public class MessagePartnerV2Controller {
    ...
    @Operation(summary = "Submits a new message (deprecated - replaced by partner API v3)", deprecated = true,
    ...
    public ResponseEntity<Void> sendMessage(
            ...
            @RequestHeader(value = HEADER_BP_ID, required = false) @Parameter(description = "Partner identification") String bpId,
    ...
    @Operation(summary = "Returns a list of messages for the given BP (Business Partner), topicName and / or groupId (deprecated - replaced by partner API v3)", deprecated = true)
    ...
    public MessagesResultDto getMessages(
            ...
            HttpServletRequest request) throws InvalidBpIdException, MissingRequiredHeaderException {
        return getMessages(bpId, bpIdOld, topicName, groupId, lastMessageId, partnerTopic, partnerTopicOld, size, request, MessagesResultDto::createV2Dto);
    }

    private MessagesResultDto getMessages(String bpId, String bpIdOld, String topicName, String groupId,
                                          UUID lastMessageId, String partnerTopic, String partnerTopicOld,
                                          int size, HttpServletRequest request,
                                          Function<List<MessageSearchResultDto>, MessagesResultDto> dtoFactory) throws MissingRequiredHeaderException, InvalidBpIdException {
        // The nonstandard parameter name lastMessageID is accepted for backwards compatibility with the B2B hub
        if (lastMessageId == null) {
            lastMessageId = lastMessageIDFromFallbackName(request);
        }
    ...
    private static UUID lastMessageIDFromFallbackName(HttpServletRequest request) {
        String lastMessageID = request.getParameter("lastMessageID");
        return lastMessageID != null ? UUID.fromString(lastMessageID) : null;
    }

    @GetMapping(value = "/{messageId}", produces = MediaType.APPLICATION_XML_VALUE)
    @Operation(summary = "Get message (deprecated - replaced by partner API v3)", deprecated = true)
    @PreAuthorize(Roles.HAS_ROLE_READ_MESSAGE_OUT)
    ...
    public ResponseEntity<InputStreamResource> getMessage(
            ...
            @RequestHeader(value = HEADER_BP_ID, required = false) @Parameter(description = "Partner identification") String bpId,
            @RequestHeader(value = HEADER_BP_ID_OLD, required = false) @Parameter(description = "Partner identification") String bpIdOld
    ) throws InvalidBpIdException, MissingRequiredHeaderException {
    ...
    @Operation(summary = "Get next message (deprecated - replaced by partner API v3)", deprecated = true)
    @PreAuthorize(Roles.HAS_ROLE_READ_MESSAGE_OUT)
